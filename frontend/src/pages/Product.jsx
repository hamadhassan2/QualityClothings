...
const Product = () => {
  ...
  const handleAddToCart = () => {
    ...
    // Check current quantity in cart specifically for this variant.
    const currentQty =
  ...
  return (
    <div
      className={`border-t pt-10 max-w-7xl mx-auto px-4 transition-opacity duration-700 ${
        animate ? "opacity-100" : "opacity-0"
      }`}
    >
      <div className="flex flex-col md:flex-row gap-12">
        <div className="flex-1 flex flex-col-reverse md:flex-row">
          {/* Thumbnail Images Container - Reduced height on mobile */}
          <div className="flex flex-wrap md:flex-col overflow-x-auto md:overflow-y-auto md:w-1/5 w-full h-48 md:h-auto border-r pr-2">
        ...
        <div className="flex-1 space-y-6 text-left">
          ...
          {/* Price Display - Shows both discounted and actual price if discount exists */}
          ...
          <div>
            <span
              className={`inline-block text-xs font-medium px-3 py-1 rounded ${
                Number(productData.count) > 0
                  ? 'bg-gray-200 text-gray-800'
                  : 'bg-gray-200 text-gray-600'
              }`}
            >
              {Number(productData.count) > 0 ? 'In Stock' : 'Out of Stock'}
            </span>
          </div>
        </div>
      </div>
